...
def visualize_tree_st(graph, pos, path=None, title=""):
    ...
    if path:
        ...
        for i, node_name in enumerate(list(graph.nodes())):
...
def main():
    ...
    if uploaded_file is not None:
        ...
        codes_for_tree = unique_sorted_codes[:10]
        root = build_balanced_bst(codes_for_tree)
        G, pos = get_graph_and_positions(root)
        
        st.header("1. 🌳 Struktur Awal Tree")
        st.write("Berikut adalah 10 kode penerbangan pertama yang digunakan untuk membangun tree:")
        ...
        st.info("ℹ️ Silakan unggah file CSV di sidebar untuk memulai.")
